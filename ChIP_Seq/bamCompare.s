...
echo $WORKING_DIR/BAM/${chip}.bam
bamCoverage --bam $WORKING_DIR/BAM/${chip}.bam -o $chip_raw_coverage --binSize 100 --outFileFormat bedgraph
...
bamCoverage --bam $WORKING_DIR/BAM/${input}.bam -o $input_raw_coverage --binSize 100 --outFileFormat bedgraph
...
bamCompare -b1 $WORKING_DIR/BAM/${chip}.bam -b2 $WORKING_DIR/BAM/${input}.bam -o ${chip}_inputsubt.bw --scaleFactors $median_input:$median_chip --operation subtract --binSize 10 --extendReads 200

printf "\nRunning bamCompare ratio for $chip and $input\n"
bamCompare -b1 $WORKING_DIR/BAM/${chip}.bam -b2 $WORKING_DIR/BAM/${input}.bam -o ${chip}_ratio.bw --scaleFactors $median_input:$median_chip --operation ratio --binSize 10 --extendReads 200
